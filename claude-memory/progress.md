...
## 2026-09-10 — redig05 (remaining rheology figure series) → retrain of fresh-state models
Extraction package: redig05 recovered +494 curves / 3,334 points (54 papers, 363 mixes; flow curves 217, slump/flow vs time 94, viscosity/yield vs time ~100, G′ sweeps 26) after a verifier pass removed 38 curves (model-derived flow curves, a unit relabel, duplicates, two redig01-era fabricated/misattributed sets). master.db **19,942 curves / 116,779 points**. Finding: readable rheology curves in the corpus are now essentially exhausted — remaining presence rows are one value per mix on a dosage axis.
- `build-targets` + retrain (GroupKFold R²log / Spearman, before → after): dynamic yield 1,325 → 1,380 rows (169 papers) 0.160/0.386 → **0.188/0.401**; plastic viscosity 1,241 → 1,304 rows 0.408/0.651 → 0.362/0.635; static yield unchanged (707; new static-yield curves are rest-time series feeding Athix, not tabulated values); Athix 302 → 323 rows (50 papers) 0.141/0.394 → 0.008/0.307; mini-slump / flow table unchanged (no signal). Net: within noise for DY/PV, Athix worse with more (noisier) curve fits — the rheology ceiling stands; more papers (roadmap idea 5) are the remaining lever.

## 2026-09-11/12 — idea 5 plan A: waves 25/26 (OpenAlex 3DCP corpus) extracted, merged, models retrained
- Extraction package: 146 manifests (wave25 750 Elsevier XML papers, wave26 409 OA PDF papers) run by sonnet agents across three rate-limit interruptions; 1,150 records merged, 12 broken packets skipped, 105 excluded on full text. Curve QC before merge (sub-3-point curves → presence rows; identical arrays adjudicated on figures; equation/fit-line curves re-read; two hand-nudged values reverted). Batch workbooks w25a–e/w26a–c all SUBMITTABLE; basis audit A=0/C=0/D=0. **master.db 2,124 → 3,274 papers; mixes 22,599 → 29,610; measurements 174,565 → 236,010; curves 19,942 → 25,652 (154,473 points); 3DCP-flagged papers 109 → 1,176.** Old-wave tables verified identical to the pre-merge snapshot (a haiku pilot agent had re-run all gen scripts and wiped JSON-layer edits on 1,575 old records; restored from snapshot).
- pmpredict: `data/master.db(.xz)` refreshed (19.5 MB xz), `composition.normalize_mix` guarded against zero aggregate mass (new edge case), stale `data/raw` caches cleared, `build-features` (29,610 mixes, 27,985 normalised, 1,625 failed; schema `9dac026be72fb584`, 138 features), `build-targets`, `train --target all --variant both`. Previous summary kept as `artifacts/reports/summary_pre_w2526.md`.
- **Model changes (GroupKFold by paper, R²log / Spearman, before → after):** static_yield_stress 707→1,728 rows, 82→241 papers, **0.165/0.403 → 0.399/0.618** (no longer weak); dynamic_yield_stress 1,380→2,032 rows, 169→294 papers, 0.188/0.401 → 0.254/0.537; plastic_viscosity 0.362/0.635 → 0.405/0.648; compressive 1,325→1,634 papers, 0.422/0.688 → 0.449/0.686; flexural 0.249 → 0.288; splitting_tensile 0.011/0.242 → 0.388/0.640; water_absorption 0.324 → 0.380; initial/final setting 0.293/0.357 → 0.337/0.377. 3DCP variants now trainable for 17 targets: compressive__3dcp 41→326 papers, 0.047/0.331 → **0.416/0.573**; static_yield__3dcp 34→192 papers, 0.012/0.184 → 0.158/0.322. Unchanged/no signal: flow_table_spread, mini_slump, Athix (0.036/0.222, weak), autogenous shrinkage. Slight drops: porosity 0.232 → 0.140, hardened_density 0.318 → 0.119, cumulative_heat 0.231 → 0.169 (small pools, more heterogeneous protocols).
- Kim 2026 external check re-scored (`artifacts/reports/external/kim2026_comparison_w2526.csv`): Spearman 0.38 → 0.40, but the level bias got worse (median pred/measured 11× → 32×; 80 % PI coverage 20/20 → 7/20). The enlarged static-yield pool is dominated by printable mortars measured after rest (pool median 701 Pa), while Kim's values are 0-s-rest strain-controlled (6–40 Pa): the protocol mismatch diagnosed on 2026-09-10 now shows as a bias rather than a wide interval. Reinforces idea 2 (protocol normalisation: τ_s(0) + Athix as targets, protocol class as a feature).
- Token/agent lessons: haiku unusable for this protocol; sonnet subagents inherit the session effort and abort at "low"; short prompt + AGENT_DIGEST.md/vq.py halves per-manifest tokens but agents then skip digitisation unless told it is a deliverable. Redig06 candidates listed in `extract/batches/wave26/_revisit.txt` (33 items).
- Closed loop (`validate-design --n 200 --seed 3`, 83 usable held-out mixes; was 73 on the redig04 models): measured values inside the 80 % intervals **75 % → 84 %**, true mix's feasibility P not worse than the candidates' median **42 % → 61 %**, candidate set closer to the true composition than random feasible mixes 29 % → 35 %, exact/top-10 literature retrieval 100 %, median 7.9 s per case.

## 2026-09-12 — pre-commit refinement of waves 25/26 + retrain (PI: "커밋하기 전에 데이터를 refine")
- Extraction side (see pastemortar claude-memory/pastemortar-db.md 2026-09-12): 45-paper verifier sample → 0 wrong tabulated values, 2 serious digitisation errors + 8 minor issues fixed; 3 zero-valued papers filled; 23 old-wave papers with ambiguous units remapped; unit harmonisation moved out of records into master.db (scripts/normalise_units_db.py after merge; record-level relabel broke validator X-02 and was reverted). 8 bands 0 errors; old-wave tables identical to snapshot; A=0/C=0/D=0.
- pmpredict: data/master.db(.xz) refreshed (3,274 papers / 236,199 measurements / 25,721 curves), data/raw cleared (stale copy in data/raw_stale), `build-features` (29,626 mixes, 28,001 normalised), `build-targets`, `train --target all --variant both`. Logs: artifacts/reports/_*_refine.log.
- Model table vs the committed redig04/05 state (R²log / Spearman, GroupKFold): general compressive 44,383 rows → 47,125 (0.422/0.688 → 0.449/0.686); static yield 707 → 1,728 rows (0.165/0.403 → 0.399/0.618); plastic viscosity 1,241 → 1,817 (0.408/0.651 → 0.405/0.648); cumulative heat 447 → 617 rows (0.231/0.476 → 0.257/0.501, unit-harmonised J/g binder pool); 3DCP compressive 508 → 2,890 rows (0.047/0.331 → 0.416/0.573); 3DCP static yield 250 → 1,256 (0.012/0.184 → 0.158/0.322); 3DCP plastic viscosity 299 → 781 (0.114/0.320 → 0.108/0.379). Unit harmonisation changed no headline metric beyond noise vs the pre-refinement w25/26 retrain; the gains come from the corpus expansion.
- Not committed: PI to decide after reading the quality report.

## 2026-09-14 — Kim et al. (ACEM26 deck) re-compared against the committed w25/26 models
- Deck saved as `artifacts/reports/external/ACEM26_Kim_static_yield_buildability.pdf`; slide 9 gives 20 pastes (C70:FA20:SF10, w/b 0.30–0.50, VA 0–0.2 %, HRWRA 0–0.15 %) with static yield stress (rotational, 0 s rest) and amplitude-sweep yield-point stress. Script `scripts/kim2026_ppt_compare.py` → `kim2026_ppt_comparison.{csv,md,png}`.
- Static yield, full model with is_3dcp=1: Spearman 0.40, median pred/measured 32×, 80 % PI coverage 35 %. Same specs with **is_3dcp=0: Spearman 0.57, 1.9×** (coverage 100 % only because the band spans ~3,400×). Root cause: in the training pool the 3DCP flag separates 1,561 Pa (194 papers) from 88 Pa (50 papers) medians; only 8–15 % of rows state rest_time = 0 and ~50 % have no rest time, so the flag learned the "rested printable mortar" protocol rather than the mix family.
- Literature analogues (paste, rest 0 s) exist (75 rows / 13 papers, median 13 Pa) but are too sparse per w/b bin to rank Kim's mixes; a rest-0-only LightGBM (171 rows / 30 papers) does not rank them either. w/b effect is reproduced by the model; the HRWRA 0.05–0.15 % effect (measured ρ −0.93) is not — below the pool's 5th percentile.
- Conclusion: protocol normalisation (idea 2) is the fix; until then query lab pastes with is_3dcp=0 and treat intervals as order-of-magnitude.

## 2026-09-14 — print01: printing-process variables + printability labels (side table) for the buildability framework
- PI asked for a "nozzle + geometry → mix + print schedule + buildability" framework; order chosen: (b) label extraction first, then (a) the physics/design layer.
- Extraction package: new side table (no change to frozen records/master.db). 1,014 3DCP papers (tier1 656 + tier2 358 of 1,176) labelled by sonnet agents in 170 manifests; schema `extract/print_labels/SCHEMA.md`, validator/builder/merger scripts, DB `master/print_process.db`. Result: **7,420 print runs (printable 5,586 / borderline 932 / non_printable 902; 941 collapses, 841 stack-until-failure tests), 7,053 keyed to master.db mixes, 2,809 with layer height + reached layers/height, 1,076 whose mix has a tabulated static yield stress; 1,507 rheology-protocol rows (349 with rest time)**. Summary: `extract/batches/print01/_WAVE_SUMMARY.md`.
- Next: phase (a) — Roussel/Suiker criteria + pmpredict inverse design → schedule; calibrate on the stack-until-failure runs; Kim et al. prints as external check.
- Phase (a) started and delivered as v1 the same day: `pmpredict/buildability.py` (PrintJob / FreshMaterial → Roussel plastic collapse with linear structuration, Suiker free-wall buckling, open time, extrudability flags; schedule window; job → DesignSpec with `static_yield_stress_at_rest ≥ sf·ρgH/√3 @ rest = print time` + Athix steering; per-candidate re-assessment), `scripts/calibrate_buildability.py` → `configs/buildability_calibration.json` + `data/print_runs.parquet` + `artifacts/reports/buildability_calibration.md`, CLI `pmpredict buildability` / `init-job`, Streamlit page 빌더빌리티·스케줄, `tests/test_buildability.py`, examples `print_job_wall.yaml`, `print_job_cylinder.yaml`, `space_3dcp_design.yaml`, README section.
- Calibration findings: R = ρgH/(√3 τ_s) at collapse in 146 stack-to-failure tests (37 papers) median 1.39 [0.64, 2.87]; τ_s alone AUC 0.59 (611 runs) → physics route primary, empirical ECDF as probability band. Printable-run windows: τ_s p10/50/90 273/1186/4920 Pa, viscosity 2.7/12/57 Pa·s (failed extrusions 67 Pa·s median), layer h = 0.5·nozzle, w = 1.2·nozzle, speed 40 mm/s, cycle 68 s medians; open time median 45 min.
- Known limits: static-yield/Athix models are wide (80 % band > 1 decade) → design P(stands) ≈ 0.3 flat, candidates for a 300 mm cylinder at 20 s cycle all non_printable (models predict τ_s0 0.3–1 kPa); the schedule side (t_c ≥ 6 min) is the actionable output there. Suiker free-wall buckling is the most conservative boundary condition (toggle `check_buckling`, `wall_filaments`). Kim et al. deck has no print tests, so no external print validation yet. Nothing committed yet (PI said push later; commit on request).

## 2026-09-15 — pmpredict AI agent (채팅이 UI를 대체)
- PI 요청: 폼 UI가 제한적 → LLM 에이전트로. 결정: API 키 있으면 API, 없으면 로그인된 Claude/ChatGPT 구독; 채팅이 UI 대체(기존 폼은 "직접 실행" 토글); 도구 호출·읽기전용 SQL·보고서 생성·세션 기억.
- 구현: `pmpredict/agent/` — tools/registry(16 도구, Anthropic/OpenAI/MCP 투영), tools/normalise(폼 규칙 재구현), providers(anthropic_api, openai_api, claude_sdk[claude-agent-sdk 0.2.152, CLI 내장], codex_cli[codex exec --json + MCP], fake), core/memory/prompt, mcp_server(mcp 2.x lowlevel add_request_handler), cli(`pmpredict agent`, `agent-mcp`); `app/streamlit_app.py` 채팅 재작성 + `app/legacy_pages.py`; tests/test_agent.py(7) — 전체 빠른 테스트 39 통과; README 섹션.
- 검증: fake 백엔드로 채팅·도구 status·아티팩트 렌더 확인; MCP 클라이언트로 tools/list·call 확인; SQL 가드 동작.
- 미완: 실제 LLM 백엔드 미검증 — 이 PC에 API 키 없음, 내장 CLI는 "Not logged in"(데스크톱 앱 로그인과 별개; `claude.exe auth login` 필요), codex 미설치. `pmpredict agent --check`가 정확한 로그인 명령을 출력. ANTHROPIC_BASE_URL/CLAUDE_CODE_* 환경변수는 내장 CLI에 전달하지 않도록 처리.
- 2026-09-15 (later): PI logged the bundled CLI in (`& "...\_bundled\claude.exe" auth login`, PowerShell needs `&`). Real-model tests on claude_sdk all passed: predict (44.4 [27.9, 75.3] MPa), buildability wall verdict + schedule window + similar prints, SQL (403 VMA 3DCP papers, SYS median 1,761 Pa), literature retrieval (15 exact hits; one SpecValidationError self-corrected), session resume + flow curve + save_report (md/xlsx), design_mix 34 s (170/4096 feasible, OPC 0.5 lower bound hit), Streamlit chat turn with prompt cache hit (47k cached tokens). Cost ~$0.03–0.21 per turn reported by the SDK (subscription quota).
