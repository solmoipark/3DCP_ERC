# -*- coding: utf-8 -*-
"""pmpredict Streamlit UI.

  streamlit run app/streamlit_app.py      (or: pmpredict ui)

Pages: model status · forward prediction · inverse design · literature retrieval.
"""
...
st.set_page_config(page_title="pmpredict", page_icon="🧱", layout="wide")
...
def get_assets():
    from pmpredict.predict import Assets
    return Assets(get_cfg())


@st.cache_resource(show_spinner="문헌 저장소 로딩...")
def get_store():
    from pmpredict.design.retrieve import LiteratureStore
    return LiteratureStore(get_cfg())
...
PAGES = {"모델 현황": page_status, "성능 예측": page_predict, "유동 곡선": page_flowcurve, "구조화 곡선": page_thixo,
         "역설계": page_design, "문헌 검색": page_retrieve}
with st.sidebar:
    st.markdown("## pmpredict")
    page = st.radio("페이지", list(PAGES), label_visibility="collapsed")
    st.markdown("---")
    st.caption("문헌 DB 2,124편 · 21,650 정규화 믹스 기반. 모든 지표는 논문 단위 교차검증(GroupKFold).")
PAGES[page]()
